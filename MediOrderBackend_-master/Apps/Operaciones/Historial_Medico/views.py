...
#Reportes de pedidos
from Apps.Operaciones.Historial_Medico.serializers import PedidoDetalleSerializer
...
class ReportesPedidosApiView(PaginationMixin, APIView):
    #permission_classes = [IsAuthenticated, CustomPermission]
    model = medical_History
    ...
    def get(self, request):
        """""
        Obtener el reporte personalizado de los pedidos
        """

        logger.info("GET request to list all Medical history")
        pedidodetallado = medical_History.objects.filter(active_Patient=True)
        page = self.paginate_queryset(pedidodetallado, request)

        if page is not None:
            serializer = PedidoDetalleSerializer(page, many=True)
            logger.info("Paginated response for Medical History")
            return self.get_paginated_response(serializer.data)

        serializer = PedidoDetalleSerializer(pedidodetallado, many=True)
        logger.error("Returning all Medical History without pagination")
        return Response(serializer.data)